...
// Helper function to read games from a file
...
app.prepare().then(() => {
    ...
    chatNamespace.on("connection", (socket) => {
        ...
        socket.on("createRoom", (newRoom) => {
            ...
            chatNamespace.emit("availableRooms", rooms);  // Broadcast updated rooms list
        });
    ...
    gameNamespace.on("connection", (socket) => {
        console.log(`${socket.id} has connected to the game`);
        ...
        socket.on("createGame", ({ gameId, playerName }) => {
            ...
            socket.join(gameId);
            gameNamespace.emit("availableGames", Object.keys(games));  // Broadcast updated game list
            gameNamespace.to(gameId).emit("gameUpdated", games[gameId]);  // Notify game room of new player
        });
        ...
        socket.on("joinGame", ({ gameId, playerName }) => {
            ...
            socket.join(gameId);
            gameNamespace.to(gameId).emit("gameUpdated", game);  // Notify game room of new player
        });
        ...
        socket.on("rollDice", ({ gameId }) => {
            ...
            saveGamesToFile(games);  // Save updated game state
            gameNamespace.to(gameId).emit("gameUpdated", game);  // Notify game room of updated game state
        });
